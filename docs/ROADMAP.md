...
- [x] **13a. 구조화 리서치 결과** (07-06, 브랜치) — `src/findings.py`: frozen `Finding` + 순수
      어댑터 4종. 다이제스트 팩터·발굴·예측 섹션과 대시보드 regime/prediction JSON 이 소비
      (렌더 바이트 동일·JSON 레거시 키 보존). KR 표·국면 라인은 의도적 미적용(다컬럼/타입 경계).
- [x] **13b. 포지션 사이징 엔진** (07-06, 브랜치) — `src/portfolio.py`: 역변동성 가중(캡+재분배)
      → 상관 페널티(보유 대비) → Kelly 상한(half-Kelly, 음수 엣지 0, 현금 잔여 허용) = `propose()`.
      `weighted_backtest`(월 리밸런스, 무선견편향)로 동일가중 대비 검증 가능. ⚠️ 실네트워크 스모크 1회 권장.
- [x] **13c. 다이제스트 연동** (07-06, 브랜치) — US 발굴 종목에 "제안 N%" + 현금 범례 (best-effort,
      실패 시 비중만 생략, `--no-weights` 플래그). KR 은 가격 파이프라인 부재로 미지원.
- [x] (선택) 13d. All Weather 자산군 비중 검증 — `scripts/check_portfolio.py` §2:
      SPY/TLT/LQD/HYG/GLD/DBC/BTC 역변동성 vs 동일가중 리플레이.
