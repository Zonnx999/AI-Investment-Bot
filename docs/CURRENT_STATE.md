...
| 이슈 | 영향 | 처리 단계 |
|---|---|---|
| HTTP 재시도/백오프 없음 | 일일 배치 운영 시 transient 에러로 무작위 실패 | **3단계 (다음)** |
| 타임아웃 일관성 부재 (30s/15s/none 혼재) | yfinance hang 시 파이프라인 무한 정지 | 3단계 |
| API 키가 URL 쿼리스트링에 노출 | 에러 traceback 에 키 leak 가능 | 3단계 |
| yfinance 자체 로거가 stderr 에 ERROR 직접 출력 | `_NOISY_LOGGERS` 누름 우회됨 | 3단계 |
