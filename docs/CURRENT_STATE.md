...
## 2.5 열린 이슈 / 다음 세션 인계 (2026-06-15)

- ⚠️ **Turso 전체 재점수가 매우 느림** (US --force ~240분). 원인 = **API 아님**(데이터
  캐시 적중, fmp_ratios 2164개 캐시됨, 429/타임아웃 로그 없음). 병목은 **per-row Turso
  쓰기**(한국→us-west-2 왕복) + 행마다 `detail` JSON blob. 읽기는 로컬이라 빠름.
  → **제안 해결책**: 무거운 enrich 는 로컬 sqlite(TURSO 끄고)로 돌린 뒤 **한 번에 업로드**
  하는 경로 신설 (per-row 태평양 왕복 제거). 또는 detail 을 별도 테이블/지연저장.
- `build_universe --force` 추가됨 (freshness 무시 전체 재점수). 점수 공식 바꾸면 필요.
  단 위 느림 이슈로 클라우드 직접 --force 는 비쌈 → 로컬 재점수 후 업로드 경로가 필요.
- 점수 공식이 #3(음수배수)/#12(배당) 수정 + 4팩터로 바뀜 → DB 재점수 전엔 옛 점수 잔존.
- 작업 리듬(사용자 합의): Phase 단위로 끝낼 때마다 그 부분 리뷰. scoring/enrich/fetch 등
  '정확성 직결' 코드는 작성 직후 한 번 더 검토.
...
2. **Phase 9 KRX+DART** — ✅ 9a 발굴(한국 507종목) + 9b DART 점수(ROE/PER/PBR) 완료.
   한국도 미국처럼 저평가 발굴됨 (기아·현대차 등). build_universe 에 [3] 한국 보강 추가됨
