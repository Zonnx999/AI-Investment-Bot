...
----------------------------------------
기본 모델 id(``minimaxai/minimax-m2``)와 응답 스키마(choices[0].message.content)는
...
def _normalize(content: str) -> str:
    """공백/개행 정리 + 최대 길이 강제 (초과 시 '…' 로 절단)."""
    text = " ".join(content.split())   # 개행·연속 공백 → 한 칸 (한 문단 유지)
...
def summarize(digest_text: str) -> str:
    ...
    try:
        response = get_http_session().post(url, json=payload, headers=headers)
