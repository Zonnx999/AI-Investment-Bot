...
- **Python 3.12** (3.10+ 호환)
- 가상환경: `.venv/` (프로젝트 루트)
- 패키지 관리: `requirements.txt` (5단계 리팩토링에서 `pyproject.toml` 로 전환 예정)
...
### 4.4 모듈 책임 분리 원칙
- `src/data_fetcher.py` — **모든** 외부 API 호출 전담 (다른 모듈은 여기서만 데이터 받음)
...
### 4.9 보안
- 코드/로그/에러 메시지에 **API 키가 노출되지 않도록** 주의 (현재 `_fmp_get` 은 키가 URL 쿼리스트링에 들어가서 traceback 에 노출될 수 있음 — 3단계 리팩토링 대상)
